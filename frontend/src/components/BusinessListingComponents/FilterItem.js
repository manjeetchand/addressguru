...
const FilterItem = ({
  ...
  return (
    <div className="relative">
      ...
      {/* {isRadio && isOpen && radioOptions.length > 0 && (
        <div className="absolute top-full left-0 z-50 mt-2 w-56  bg-white  rounded text-xs">
          <div className="p-3 space-y-2  max-h-64 overflow-y-auto">
            {radioOptions.map((option) => (
              <label
                key={option.id}
                className="flex items-center gap-1 cursor-pointer  hover:bg-orange-50  rounded transition-colors"
              >
                <div className="relative flex-shrink-0">
                  <input
                    type="radio"
                    name={`radio-${index}`}
                    checked={tempSelectedId === option.id}
                    onChange={() => setTempSelectedId(option.id)}
                    className="sr-only r"
                  />
                  <div
                    className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all
                      ${
                        tempSelectedId === option.id
