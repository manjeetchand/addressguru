...
const FilterBar = ({
  ...
  const handleFilterSelect = (label, value = null) => {
    switch (label) {
      case "Sort by":
        onFilterChange({
          sort_by: value?.toLowerCase(), // newest | oldest | popular
        });
        break;

      case "Facilities":
        // ✅ FIX: value now contains only the selected facilities
        onFilterChange({
          facilities_id: value?.map((facility) => facility?.id) || [],
        });
        break;

      case "Services":
        onFilterChange({
          services_id: value?.map((service) => service?.id) || [],
        });
        break;

      case "Payment Mode":
        onFilterChange({
          payment_mode_id: value?.map((paymentMode) => paymentMode?.id) || [],
        });
        break;

      case "AG Verified":
        onFilterChange({
          ag_verified: !filters.ag_verified,
        });
        break;
  ...
  return (
    <div className="flex items-center max-md:bg-gray-100 max-md:py-1  md:w-[920px] px-2 py-1 max-md:w-[90vh] max-md:max-w-[350px] max-md:ml-2 max-md:my-2 rounded-lg">
      {/* the initial svg code for the filter icon */}
      <svg
        className="max-md:hidden border border-gray-300 required-md"
        width="34"
        height="34"
        viewBox="0 0 38 38"
        fill="none"
      ...
      <div
        ref={containerRef}
        className="flex gap-3  max-md:overflow-x-scroll hide-scroll   md:pl-4"
      >
        {filterItems.map((item, index) => (
          <FilterItem
            ...
            // For radio select (if you have any)
