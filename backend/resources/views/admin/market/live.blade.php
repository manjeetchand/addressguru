 ...
 <table class="table table-striped table-bordered">
    ...
    <tbody>
      ...
      <tr>
        <td>{{$i}}</td>
        <td>
          <img src="{{url('/')}}/images/{{isset($key->medias[0]) ? $key->medias[0]->name : 'user.png'}}" alt="{{$key->title}}" class="img-responsive" style="max-width:100%;height:80px;">
        </td>
       ...
       <td>{{$key->status ? 'Published' : 'Under-Review'}}</td>
         ...
         <td><a href="{{route('admin-marketplace.edit', $key->id)}}" style="padding:1px 4px 1px 4px;font-size:12px;" class="btn btn-primary">Un-Approve</a></td>
