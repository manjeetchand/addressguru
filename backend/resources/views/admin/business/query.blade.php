...
<div class="table-responsive">
    <table class="table table-striped table-bordered" >
        ...
        <tbody id="myTable">
            ...
            <tr id="row{{$i}}">
                <td>{{$i}}</td>
                <td><a href="{{url('/preview', $key->slug)}}">{{$key->business_name}}</a><br>
                    <small>{{$key->user->name ?? ''}} || {{date('d F, Y', strtotime($key->created_at))}}</small>
                 </td>
                @if($type === 'query')
                <td><a href="{{route('admin-query.edit', $key->id)}}">View Queries ({{0}})</small></a> </td>
                @else
                <td><a href="{{route('admin-query.edit', $key->id)}}">Report ({{ 0}})</small></a> </td>
                @endif
                {{-- <td>{{$key->personals[0]->ph_number}}</td>
                <td>{{$key->personals[0]->state}}</td>
                <td>{{$key->personals[0]->city}}</td> --}}
                <td>
